...
uit officiële systemen worden geïmporteerd. Hiermee wordt het aanvraagproces versimpeld en wordt de kans op fouten of
fraude kleiner.
...
Om de hoeveelheid configuratie tot een minimum te beperken kunnen plug-ins inhaken op voorgedefinieerde systeem velden:

![Veld definities](./images/field-defs.svg)
...
een bepaald type is. Een plug-in kan in zijn definitie bepaalde veld groepen vereisen om te functioneren. Bij het 
...
standaard [OAuth](https://oauth.net/2/) implementatie wordt teruggekoppeld of dit succesvol is geweest en wie de
gebruiker betreft. Dit kan een eenvoudige identifier zijn, maar het is ook mogelijk in combinatie met de automatisch
aanvullen functionaliteit bijv. NAW-gegevens door te geven.
...
dat de veldwaardes automatisch zijn ingevuld vindt er validatie plaats aan de achterkant dat deze waardes overeenstemmen
met de waardes die op dat moment via de koppeling worden teruggegeven voor de identificerende velden. Mocht er in de
tussentijd iets gewijzigd zijn in deze gegevens, of de gebruiker heeft op één of andere manier de gegevens weten 
aan te passen zonder dat het systeem dit heeft gezien als aanpassing door de gebruiker, dan zal dit als validatie fout
worden teruggegeven en dient de gebruiker dit deel van het formulier opnieuw in te vullen.
...
behandelproces zitten regelmatisch gesynchroniseerd worden met het externe systeem.  
...
het systeem een suggestie doet voor de bankrekeninghouder indien de ingevoerde waarde net niet klopt. 

Deze koppeling is reeds gerealiseerd als onderdeel van het validatie- en beoordelingsproces. Op dit moment is dit
...
Het aanvraagportaal ondersteund [DigiD](https://www.digid.nl) via [MAX]((https://github.com/minvws/nl-rdo-max) 
...
* **Defense in depth**; Indien het om gevoelige gegevens gaat zoals bijvoorbeeld medische gegevens heeft een proxy API als 
  voordeel dat er een extra laag wordt gecreëerd richting deze data. Op het moment dat het systeem wat met
...
* **Versimpelen interface**; Indien een externe API speciale libraries of andere niet gangbare eisen stelt voor
  communicatie kan een proxy API uitkomst bieden. De proxy API hoeft maar één keer te worden opgezet waarna 
  verschillende projecten gebruik kunnen maken van een meer toegankelijke REST/JSON interface. 
...
met het kunnen extraheren naar een aparte library repository. Op deze manier kan deze API _client_ binnen diverse
projecten worden hergebruikt. Uiteraard dient er eerst te worden bekeken of een dergelijke abstractie niet al reeds
gerealiseerd is binnen de scope van een ander project of eventueel Open Source beschikbaar is. Ook wordt het 
aangemoedigd om eventueel zelf ontwikkelde API clients voor (semi-)publieke APIs te Open Sourcen.
...
[TVS](https://www.digitaleoverheid.nl/overzicht-van-alle-onderwerpen/identiteit/toegangsverleningservice/)
(ToegangVerleningService). TVS maakt het mogelijk om via configuratie ook eHerkenning te activeren.
...
van bedrijfsgegevens. Op basis van het KVK nummer kunnen ondermeer de bedrijfsnaam en het vestigingsadres automatisch
worden ingevuld.
...
De BRP koppeling zal binnen de context van DUS-i worden gebruikt voor het automatisch in-/aanvullen van
persoonsgegevens. Op basis van het BSN kunnen de NAW gegevens voor een persoon worden ingevuld.
...
op te vragen. Het opvragen van deze gegevens kan via Dit kan via [LRZa](https://www.cibg.nl/lrza) of 
