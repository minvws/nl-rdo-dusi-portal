 ...
 */
class HsmLocalClearCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'hsm:local-clear';
     ...
     * @var string
     */
    protected $description = 'Clear the local HSM objects (only for development)';
    ...
    {
        if ($this->environment !== 'local' || !$this->debugModeEnabled) {
            $this->error('Command is only intended for local development.');
            return;
        }
    ...
    protected function isLocalAndDebug(): bool
    {
        return $this->environment !== 'local' || !$this->debugModeEnabled;
    }
}
